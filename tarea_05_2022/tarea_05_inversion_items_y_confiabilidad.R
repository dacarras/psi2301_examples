...
# -----------------------------------------------
# analisis de items, con items recodificados
# -----------------------------------------------

data_model %>%
dplyr::select(i1, i2, i3, i4, i5, i6) %>%
psych::alpha(., check.key = TRUE)

# Nota: psych::alpha() asume que la direccion de la
#       escala es a más sexismo.

# -----------------------------------------------
# analisis de items, con items invertidos
# -----------------------------------------------

data_model %>%
dplyr::select(i1, i2, i3, i4r, i5r, i6r) %>%
psych::alpha()

# Nota: psych::alpha() cuando los items son invertidos,
#       por el usuario, en la direccion deseada, el
#       alpha estimado, y los indicadores de item-test
#       son calculados en la direccion del puntaje esperado,
#       a mayor puntaje, mayor apoyo a la equidad entre hombres
#       y mujeres.
